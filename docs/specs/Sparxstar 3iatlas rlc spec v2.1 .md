# 3iATLAS RAPID LANGUAGE COLLECTION (RLC) *Technical Specification v2.1*
...
- v2.1 adds Section 12 (Runtime Architecture Doctrine) and Section 13 (Runtime Event Contract). All other sections are unchanged from v2.0.

---

## **1\. Overview & Mission**

The 3iAtlas Rapid Language Collection (RLC) is a classroom-deployable web application that collects structured linguistic data through guided gameplay. It is a standalone tool — it does not depend on DVE or any SPARXSTAR runtime pipeline at runtime. It runs anywhere a modern browser runs.
...
* Standalone: React + Node, no DVE runtime dependency, no live SPARXSTAR pipeline connection

* **WordPress is the deployment host.** WordPress is not the architecture. RLC is a Node + React application. WordPress provides authentication, user management, and multisite isolation. It does not own the RLC runtime, data model, or event substrate.
...
* Not a WordPress application — WordPress is the host, not the framework. RLC logic does not use WP hooks, WP options, WP post types, or WP query as its primary data layer.
...
| Hosting: WordPress as deployment host. Node process runs alongside WordPress. WordPress handles auth and user management only. |
...
| **packages/shared/** | TypeScript types shared between client and server. Session schema, token types, domain lists. **Also contains: RlcEvent types (Section 13).** |
...
| Runtime events (Section 13) are queued locally alongside submissions and replayed on reconnection. |
...
**IMPORTANT — Ability boundary:** The AI facilitator is invoked through `invokeAbility('rlc.facilitate')`. RLC never assembles prompts directly in surface components. The session parameters (language, domain, mode) are passed as structured context to the ability boundary. The ability implementation constructs the prompt. This keeps cognition centralized. See Section 12.3.
...
| T2 | Live Monitor | Large join code + QR. Live participant count. Rolling submission feed. Live leaderboard by XP. End session button. **Connectivity status per participant (online / offline / sync-queued).** |
...
| S2 | RWC Collection | Three sequential panels: (1) Word entry with wordpad; (2) Translation; (3) Starmus recorder. XP counter in header. Submit active only when required steps done. **Sync status indicator in header.** |
| S3 | RSC Collection | Same structure as S2 but shows sentence prompt with grammar domain context card. Focus word underlined red as student types. Progress indicator: 7 of 12\. **Sync status indicator in header.** |
...
**UI Overlay Rules (v2.1 addition):**

* Every student screen shows a persistent sync status indicator. Three states: `online` (cloud icon), `offline` (no-connection icon), `syncing` (animated upload icon).
* Submissions made while offline show a `queued` badge. Badge clears when server confirms receipt.
* Confidence indicators: spelling signal is surfaced visually. `confirmed` = green dot, `variant` = amber dot, `discovery` = gold star. No text label required — the icon is enough.
* Speaker affirmation: if a submission was recorded AND passed QC vote, show an affirmation badge on the submission in the teacher monitor.
...
| Method + Path | Auth | Description |
| :---- | :---- | :---- |
| POST /token/save | participant\_id | Save a submission. Body: { session\_id, participant\_id, text, translation?, collection\_mode, grammar\_domain?, audio\_metadata? }. Returns: { token\_id, spelling\_signal, saturation\_signal }. **audio\_metadata carries: { duration\_ms, speaker\_id, recorded\_at } — never the audio blob itself.** |
...
## **7.4 Event Endpoint (v2.1 addition)**

| Method + Path | Auth | Description |
| :---- | :---- | :---- |
| POST /events/batch | participant\_id or Teacher JWT | Flush queued runtime events. Body: { events: RlcEvent\[\] }. Returns: { accepted: number, failed: number }. Used by the offline sync queue. Events are append-only — the server never modifies or deletes them. |
...
| **audio\_duration\_ms** | Integer. Duration of the recording in milliseconds. NULL if no recording. |
| **audio\_speaker\_id** | Participant ID of the speaker. NULL if no recording. Always the submitter. |
| **audio\_recorded\_at** | Unix timestamp of recording. NULL if no recording. |
...
| **speaker\_affirmed** | Boolean. True when the submission has audio AND passed the QC vote. Highest confidence signal. |
...
| Audio metadata (duration, speaker, recorded\_at) is canonical. The audio file itself is stored by Starmus. |

## **8.3 Runtime Event Log (v2.1 addition)**

Runtime events are stored in an append-only `rlc_events` table. Events are never updated or deleted. They are the audit substrate for every significant action in a session.

| Field | Type / Notes |
| :---- | :---- |
| **event\_id** | UUID, primary key |
| **event\_type** | String — one of the canonical event types in Section 13 |
| **session\_id** | UUID — foreign key → session |
| **participant\_id** | String — who emitted this event |
| **payload** | JSONB — event-specific data |
| **emitted\_at** | Unix timestamp — set by the client at emission time |
| **received\_at** | Unix timestamp — set by the server at receipt time |
| **sequence** | Integer — monotonically increasing within a session, set by client |
...
| :---- | :---- | :---- |
| 1 — Scaffold | Monorepo setup. packages/client (Vite + React + TS), packages/server (Node + Express), packages/shared (types including RlcEvent). PostgreSQL schema including rlc\_events. socket.io wired. Dictionary JSON loaded. | npm run dev starts both client and server with no errors. |
| 2 — Session core | POST /session/create. POST /session/join. WebSocket: session:join event. Teacher T2 screen (join code + QR + live participant count). Student S1 screen (join). Basic connectivity. Runtime event: RLC\_SESSION\_STARTED emitted. | Teacher creates session. Student joins on phone. Teacher sees student appear on monitor. |
| 3 — RWC Collection | Student S2 screen. Wordpad mounted. Special character bar (ŋ ɓ ɗ ñ). Three-step state machine (typed → translated → recorded). POST /token/save. Spelling signal returned. Teacher T2 rolling feed. Runtime events: RLC\_WORD\_CAPTURED, RLC\_AUDIO\_ATTACHED, RLC\_TRANSLATION\_ADDED emitted on each action. | Student submits a word in Mandinka with translation and audio. Teacher sees it appear on monitor in real time. Events persisted in rlc\_events. |
...
| 5 — QC Phase | GET /session/:id/qc-words (selection algorithm). Teacher T3 screen. Student S4–S7 screens. Vote endpoints. Translation collection. Correction flow. All real-time via WebSocket. Runtime event: RLC\_SPEAKER\_AFFIRMED emitted when submission passes QC vote. | Full QC round completes: word displayed, votes cast, translation collected, next word advances. |
...
| 7 — Polish | PWA manifest. Offline submission queue (IndexedDB) + event queue. POST /events/batch endpoint. Sync status UI overlays. Connectivity indicators. Error handling. Loading states. End-to-end test: one full session both modes. | App installs as PWA. Submission and events survive a 30-second network drop. Full session test passes. |
...
| **WordPress is the host, not the framework** | RLC is a Node + React application. WordPress handles auth and user management. RLC does not use WP hooks, WP post types, or WP options as its data layer. |
...
| **Audio is canonical** | Audio is the primary artifact. Text is a projection of audio, not the other way around. Audio metadata (speaker, duration, timestamp) is stored with every token that has a recording. The audio file is stored by Starmus. RLC stores the metadata reference. |
| **Runtime events are append-only** | Events in rlc\_events are never updated or deleted. A correction event adds a new row. It does not modify the original event row. |
| **Ability boundary for AI** | RLC never assembles AI prompts in surface components. All AI invocation goes through invokeAbility(). Structured session context is passed to the boundary. The ability implementation owns the prompt. |
...
| **Full offline mode** | Phase 7 adds offline submission queuing. Full offline (no server at all) is a future phase. Event replay from the local queue is the substrate that makes this possible when it arrives. |
...
| **Full runtime graph** | The event log is the substrate. Full event replay, lineage visualization, and authority-weighted confidence overlays are future phases built on top of the substrate. Do not build them now. |
| **Regional Brain integration** | Future. RLC will feed Regional Brain training via the export pipeline. No live connection during this phase. |

---

## **12\. Runtime Architecture Doctrine (v2.1 — New)**

This section defines the architectural boundaries that keep RLC aligned with the SPARXSTAR sovereign runtime architecture. These boundaries do not add features to this sprint. They prevent architectural debt that would block the runtime future.

## **12.1 WordPress Is the Host**

WordPress provides: authentication (JWT via WP user system), user management, multisite isolation, and the hosting environment.

WordPress does not provide: the RLC data model, the session runtime, the event substrate, the API layer, or the real-time layer.
...
WordPress
  └── hosts Node process
        └── Node/Express owns the RLC runtime
              ├── PostgreSQL owns the RLC data
              ├── socket.io owns the real-time layer
              └── IndexedDB owns the offline queue
```

No RLC business logic lives in PHP. No RLC data lives in WP post types or WP options. WP authentication tokens are validated by the Node server — the Node server is the authority for session state.

## **12.2 Audio Is Canonical**

Audio is the primary artifact. Every other field in a token — text, translation, corrections, votes — is a projection derived from or about the audio.

This has concrete implementation implications:

- `audio_id`, `audio_duration_ms`, `audio_speaker_id`, `audio_recorded_at` are stored with every token that has a recording. These fields are never null for Full mode submissions.
- The audio file itself is stored by Starmus. RLC stores the metadata reference. RLC never stores raw audio blobs in its own database.
- `speaker_affirmed` is set to true when: (a) a recording exists, AND (b) the submission passed the QC vote. This is the highest-confidence signal a token can carry. It means: a speaker said this word, and the community agreed it was correct.
- Future confidence overlays, dialect clustering, and training provenance all derive from audio metadata. Designing around text strings now would require expensive retrofitting later.

## **12.3 Ability Boundary for AI**

RLC never directly assembles AI prompts in surface components or API route handlers. All AI invocation follows the ability contract pattern:

```typescript
// CORRECT
await invokeAbility('rlc.facilitate', {
  session_id,
  language,
  semantic_domain_id,
  mode: 'rwc',
  submission_text: token.text,
  spelling_signal: token.spelling_signal,
});

// WRONG — never do this
const prompt = `You are a linguist learning ${language}...`;
await fetch('/ai/chat', { body: { prompt } });
```

For this sprint, `invokeAbility()` is a thin wrapper that calls the LibreChat API internally. The wrapper signature is what matters — the implementation can be simple. The boundary prevents surface-local prompt assembly from becoming the pattern.

`invokeAbility()` lives in `packages/server/src/abilities/`. It accepts structured context. It returns structured results. It never returns raw AI text to the client — it returns parsed, typed results.

## **12.4 Runtime Events Are the Substrate**

The `rlc_events` table is not a log. It is the substrate. Every significant action in a session emits an event. Events are append-only. Events are the source of truth for what happened in a session.

The concrete implication for this sprint: any feature that currently stores state in a mutable field should also emit an event. The mutable field is for current state. The event log is for history and replay.

Future capabilities that depend on the event substrate being in place from the start:

- Offline replay: when a student reconnects, their queued events are flushed and the session state is reconstructed.
- Session audit: what happened in this session, in order, with timestamps.
- Training provenance: which submissions were speaker-affirmed, in what order, by whom.
- Solo/offline mode (future): the event queue replays deterministically when the server is unavailable.

Do not wait until "Polish" to add events. Events belong in Phase 1 scaffold alongside the schema.

## **12.5 Offline Queue Architecture**

The offline queue is not a backup mechanism. It is the primary submission path. Every submission — whether the device is online or offline — goes through the queue.

```
Student action
  → append to IndexedDB queue
    → attempt immediate flush to server
      → if success: mark as synced, emit RLC_SYNC_QUEUED then RLC_SYNC_COMPLETE
      → if failure: leave in queue, retry on reconnection
```

This means: the submission never fails from the student's perspective. They submit. It goes in the queue. The queue eventually flushes. The server confirms.

The UI reflects queue state — not network state. `online`, `offline`, and `syncing` are queue states, not connectivity states. A device can be online but still syncing a backlog.

---

## **13\. Runtime Event Contract (v2.1 — New)**

This section defines the canonical runtime events emitted by RLC. These events are the formal contract between the RLC runtime and any future consumer (export pipeline, Regional Brain, replay substrate, training provenance system).

All event types are defined as TypeScript enum values in `packages/shared/src/events.ts`. The server validates incoming events against this enum. Unknown event types are rejected.

## **13.1 Canonical Event Types**
...
All events share the same base shape. The `payload` field carries event-specific data.
...
export interface RlcEvent {
  ...
  emitted_at:  number;       // Unix timestamp — client clock
  sequence:    number;       // Monotonically increasing within participant session
  payload:     Record<string, unknown>;
}
```

## **13.3 Key Payload Shapes**

```typescript
// RLC_WORD_CAPTURED
{ text: string; language: string; semantic_domain_id: string; }

// RLC_AUDIO_ATTACHED
{ token_id: string; audio_id: string; duration_ms: number; }

// RLC_TRANSLATION_ADDED
{ token_id: string; translation: string; language_target: string; }

// RLC_SPEAKER_AFFIRMED
{ token_id: string; audio_id: string; vote_yes_count: number; vote_no_count: number; }

// RLC_SYNC_QUEUED
{ queued_event_ids: string[]; queue_depth: number; }
```

## **13.4 Event Rules**

- Events are emitted at the moment of action, not at save confirmation.
- Events are queued locally (IndexedDB) before transmission. The server receives them via POST /events/batch.
- The server assigns `received_at` on receipt. `emitted_at` is the client timestamp.
- Events are never modified after creation. A correction to a word creates `RLC_CORRECTION_SUBMITTED`, not a modification of `RLC_WORD_CAPTURED`.
- `sequence` is scoped per participant per session. It starts at 1. It never resets within a session.

---

## **14\. Solo Mode and Play vs AI (Future Architecture Note)**

These modes are explicitly deferred from this sprint but the architecture must not block them.

Solo mode: a student plays without a teacher session. They self-create a session. No QC phase (no class). Awards are instant and solo. The event substrate and offline queue handle this identically to a class session.

Play vs AI mode: AI fills opponent slots. Students compete against AI-generated submissions. The AI submissions are generated through `invokeAbility('rlc.ai_opponent')`. The AI never directly accesses the session data — it receives the semantic domain and grammar domain as structured context and returns typed submission objects.

**The one rule that protects these future modes:** Do not hardcode the teacher-required session flow as the only path through the system. The session state machine must accommodate sessions without a teacher\_id. Check for the existence of a teacher participant — do not assume one always exists.

---

*End of SPARXSTAR-3iAtlas-RLC-Spec-v2.1*

*Starisian Technologies · May 2026 · Confidential*
